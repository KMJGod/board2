...
public class SearchBoardRepositoryImpl extends QuerydslRepositorySupport implements SearchBoardRepository {
    ...
    public Board search1() {
        log.info("search1() 호출_____________________");
    ...
    public Page<Object[]> searchPage(String type, String keyword, Pageable pageable) {
        log.info("searchPage() 호출_____________");
        QBoard board = QBoard.board;
        QMember member = QMember.member;
        QReply reply = QReply.reply;
        ...
        if(type != null){
            ...
            for (String t: typeArr){
                switch (t){
                    case "t":
                        condtionBuilder.or(board.title.contains(keyword));
                        break;
                    case "w":
                        condtionBuilder.or(member.email.contains(keyword));
                        break;
                    case "c":
                        condtionBuilder.or(board.content.contains(keyword));
                        break;
                }
            }
